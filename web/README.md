# GovBid client dashboard (Next.js)
...
Mock RFPs live in `lib/mockData.ts`. Profile fields and saved RFP IDs persist to `localStorage` under the key `govbid-dashboard-v1` (older installs are migrated automatically on first load).